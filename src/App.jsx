...
function App() {
  ...
  const contractAddress = "0xF896bB1Da84b8dDE7Ca31D79075B56e51Cdd5582";
  const contractABI = [
    {
      "inputs": [
        {
  ...
  const loadOwnedItems = async (contract, owner) => {
    ...
    let ownedItems = [];
    for (let i = 1; i < ownedItemsIDs.length; i++) {
  ...
  const tranferItem = async (id, toAddress) => {
  ...
  return (
    <>
      ...
      <div className='list-item'>
        ...
        <input id="itemPrice" placeholder='Item Price' className='input-field' />
        ...
        {items.map((item) => {
          <div key={item.id} className='item-card'>
            ...
            {!item.isSold && item.owner.toLowerCase() !== account.toLowerCase() && (
              <button className='button' onClick={() => purchaseItem(item.id, ethers.utils.formatEther(item.price))}>Purchase</button>
            )}
          </div>
        })}
      </div>
      ...
      <div className='owned-items'>
        ...
        {items.map((item) => {
          <div key={item.id} className='item-card'>
            ...
            <button className='button' onClick={() => transferItem(item.id, document.getElementById(`transferAddress${item.id}`).value)}>Transfer</button>
          </div>
        })}
      </div>
    </>
